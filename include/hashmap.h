...
static inline size_t long_hash(long l)
{
	return l ^ 11400714819323198485llu;
}

static inline bool long_cmp(long l1, long l2)
{
	return l1 == l2;
}
...
#define hashmap_cast_ptr(p)                                                      \
	({                                                                       \
		_Static_assert(                                                  \
			(__builtin_constant_p((p)) ? (p) == NULL : 0) ||         \
				sizeof(*(p)) == sizeof(long),                    \
			#p                                                       \
			" pointee should be a long-sized integer or a pointer"); \
		(long *)(p);                                                     \
	})
...
#define hashmap__set(map, key, value, old_key, old_value)              \
	hashmap__insert((map), (key), (value), HASHMAP_SET, (old_key), \
			(old_value))

#define hashmap__update(map, key, value, old_key, old_value)              \
	hashmap__insert((map), (key), (value), HASHMAP_UPDATE, (old_key), \
			(old_value))
...
#define hashmap__delete(map, key, old_key, old_value)                 \
	hashmap_delete((map), (long)(key), hashmap_cast_ptr(old_key), \
...
#define hashmap__for_each_key_entry(map, cur, _key)                       \
	for (cur = map->buckets ? map->buckets[hash_bits(                 \
					  map->hash_fn((_key), map->ctx), \
					  map->cap_bits)] :               \
				  NULL;                                   \
	     cur; cur = cur->next)                                        \
